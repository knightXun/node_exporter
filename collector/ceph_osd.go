...
func init() {
	...
	kingpin.Parse()
	if *enable_ceph {
		registerCollector("ceph_monitor", defaultEnabled, NewOSDCollector)
	}
}
...
func NewOSDCollector() (Collector, error) {
...
func (o *OSDCollector) Update(ch chan<- prometheus.Metric) error {
	if err := o.collectOSDPerf(); err != nil {
		log.Println("failed collecting osd perf stats:", err)
		return err
	}
